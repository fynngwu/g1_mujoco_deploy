...
IOSDK::IOSDK()
{
    ...
    wireless_subscriber_.reset(new ChannelSubscriber<unitree_go::msg::dds_::WirelessController_>("rt/wirelesscontroller"));
    wireless_subscriber_->InitChannel(std::bind(&IOSDK::WirelessControllerHandler, this, std::placeholders::_1), 1);
...
void IOSDK::LowStateHandler(const void *message)
{
    ...
    if (gamepad_.L2.pressed && gamepad_.up.pressed)
    {
        userCmd_ = UserCommand::START;
    }
    if (gamepad_.R1.pressed && gamepad_.X.pressed)
    {
        userCmd_ = UserCommand::R2_A;
    }
    if (gamepad_.L2.pressed && gamepad_.down.pressed)
    {
        userCmd_ = UserCommand::R1_UP;
    }
    if (gamepad_.L2.pressed && gamepad_.left.pressed)
    {
        userCmd_ = UserCommand::R2_B;
    }
...
void IOSDK::WirelessControllerHandler(const void *message)
{
    auto &msg = *(const unitree_go::msg::dds_::WirelessController_ *)message;
    ...
    gamepad_.R1.update(btn.components.R1);
    gamepad_.L1.update(btn.components.L1);
    gamepad_.start.update(btn.components.start);
    gamepad_.select.update(btn.components.select);
    gamepad_.R2.update(btn.components.R2);
    gamepad_.L2.update(btn.components.L2);
    gamepad_.F1.update(btn.components.F1);
    gamepad_.F2.update(btn.components.F2);
    gamepad_.A.update(btn.components.A);
    gamepad_.B.update(btn.components.B);
    gamepad_.X.update(btn.components.X);
    gamepad_.Y.update(btn.components.Y);
    gamepad_.up.update(btn.components.up);
    gamepad_.right.update(btn.components.right);
    gamepad_.down.update(btn.components.down);
    gamepad_.left.update(btn.components.left);
